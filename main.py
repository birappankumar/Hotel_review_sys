...
from tkinter import *
import sqlite3
# Building the model from the tsv file review
...
db_file = 'Restaurant_food_data.db'

if not os.path.exists(db_file):
    init_data()  # Create the table if the file doesn't exist
...
# Processing and storing the data form user review
def estimate(s,cursor):
...
# Collecting Data code for collecting data use of this function is below while taking input from custumer
def take_review():
	root2 = Toplevel()
	...
	submit_review = Button(root2, text="Submit Review", font=(
		...
		estimate(rev_tf.get(),c), root2.destroy()])
...
# function for seeing the details from database
def view_details(s):
	# if(s != rras_code):
	# 	popup()
	# else:
...
# Verifying Ownership if the owner want to modifie the input data from user
def login():
	root3 = Toplevel()
...
# Creating the gui for taking input from user and owner
root1 = Tk()
